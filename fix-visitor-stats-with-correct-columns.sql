-- ============================================
-- 방문자 통계 로직 수정 (올바른 컬럼명 사용)
-- 문제: 날짜별/시간대별 방문자가 중복 카운트됨
-- 해결: visitor_ip + user_agent 조합으로 고유 방문자 카운트
-- ============================================
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'stats', json_build_object(
      -- 오늘 고유 방문자 (visitor_ip + user_agent 조합)
      'today', (
        SELECT COUNT(DISTINCT (visitor_ip || '|' || COALESCE(user_agent, '')))
        FROM visitor_logs
...
BEGIN
  -- 기존 정책이 있으면 삭제
  DROP POLICY IF EXISTS "Allow admin to view all logs" ON visitor_logs;
...
-- 6. 검증 쿼리
-- 오늘 고유 방문자 수 확인 (visitor_ip + user_agent 조합)
SELECT 
  '오늘 고유 방문자' as label,
  COUNT(DISTINCT (vl.visitor_ip || '|' || COALESCE(vl.user_agent, ''))) as count
FROM visitor_logs vl
