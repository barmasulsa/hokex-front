-- Update function to include venue_event_page_url parameter
-- This adds support for storing the venue's event page URL (e.g., COEX event page)

CREATE OR REPLACE FUNCTION update_event_scraping_result(
  p_event_id UUID,
  p_poster_url TEXT,
  p_venue_event_page_url TEXT,  -- NEW: 전시장 행사 페이지 URL
  p_description TEXT,
...
BEGIN
  ...
  SET
    ...
    venue_event_page_url = COALESCE(p_venue_event_page_url, venue_event_page_url),  -- NEW
...
COMMENT ON FUNCTION update_event_scraping_result IS 
  ...
  'Includes venue_event_page_url for storing the venue''s event page URL. ' ||
