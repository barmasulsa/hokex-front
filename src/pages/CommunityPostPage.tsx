...
export function CommunityPostPage() {
  const { postId } = useParams(); const { user } = useAuth(); const navigate = useNavigate(); const [post, setPost] = useState<Post | null>(null); const [error, setError] = useState('');
  ...
  return <main className="community-page"><article className="post-detail"><Link to="/community" className="back-link">← 커뮤니티 목록</Link><div className="post-detail-title"><span className="post-tag">COMMUNITY</span><h2>{post.title}</h2><p>{post.author_nickname || '익명 판다'} · {new Date(post.created_at).toLocaleString('ko-KR')} · 조회 {post.view_count}</p></div><div className="post-detail-content"><RichTextContent value={post.content} /></div>{user && <div className="post-owner-actions"><button onClick={() => navigate(`/community/${post.id}/edit`)}>수정</button><button className="danger" onClick={remove}>삭제</button></div>}<section className="comments-placeholder"><h3>댓글</h3><p>댓글·추천 기능은 다음 단계에서 추가됩니다.</p></section></article></main>;
}
