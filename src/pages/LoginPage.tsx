...
const PRIVACY_VERSION = '2026-05-20';

export function LoginPage() {
  const { user, loading, signInWithGoogle, signInWithNaver, signInWithPassword, signInWithMagicLink, signUp, resetPassword } = useAuth();
  ...
  const [agreed, setAgreed] = useState(false); const [marketingAgreed, setMarketingAgreed] = useState(false); const [ageOver14, setAgeOver14] = useState(false); const [emailOpen, setEmailOpen] = useState(false); const [error, setError] = useState(''); const [notice, setNotice] = useState(''); const [submitting, setSubmitting] = useState(false);
  useEffect(() => { if (!loading && user) navigate('/'); }, [user, loading, navigate]);
  const socialLogin = async (provider: 'google' | 'naver') => {
    if (!agreed || !ageOver14) { setError(!agreed ? '이용약관 및 개인정보처리방침에 동의해주세요.' : '만 14세 이상 확인이 필요합니다.'); return; }
    setSubmitting(true); setError(''); sessionStorage.setItem('hokex-open-nickname-setup', 'true');
    sessionStorage.setItem('hokex-pending-social-consent', JSON.stringify({ termsVersion: TERMS_VERSION, privacyVersion: PRIVACY_VERSION, marketingAgreed, ageOver14 }));
    try { if (provider === 'google') await signInWithGoogle(); else await signInWithNaver(); }
    catch { sessionStorage.removeItem('hokex-open-nickname-setup'); sessionStorage.removeItem('hokex-pending-social-consent'); setError(provider === 'naver' ? '네이버 로그인 설정을 준비 중입니다. 잠시 후 다시 시도해주세요.' : '소셜 로그인을 시작하지 못했습니다.'); setSubmitting(false); }
  };
  // OAuth 화면에서 로그인하지 않고 브라우저 뒤로가기를 하면 bfcache가 기존 React 상태를 복원한다.
  // 그때 남아 있던 submitting 상태가 버튼을 비활성화하고 금지 커서를 보이게 하던 문제를 해제한다.
  useEffect(() => {
    const resetCancelledSocialLogin = () => { setSubmitting(false); sessionStorage.removeItem('hokex-open-nickname-setup'); sessionStorage.removeItem('hokex-pending-social-consent'); };
    window.addEventListener('pageshow', resetCancelledSocialLogin);
    window.addEventListener('popstate', resetCancelledSocialLogin);
    return () => { window.removeEventListener('pageshow', resetCancelledSocialLogin); window.removeEventListener('popstate', resetCancelledSocialLogin); };
  }, []);
  const submit = async (event: React.FormEvent) => {
    event.preventDefault(); setError(''); setNotice('');
    if (mode === 'signup' && (!agreed || !ageOver14 || password.length < 8 || password !== passwordConfirm)) { setError(!agreed ? '이용약관 및 개인정보처리방침에 동의해주세요.' : !ageOver14 ? '만 14세 이상 확인이 필요합니다.' : password.length < 8 ? '비밀번호는 8자 이상 입력해주세요.' : '비밀번호 확인이 일치하지 않습니다.'); return; }
    setSubmitting(true);
    try { if (mode === 'signup') { await signUp(email, password, { termsVersion: TERMS_VERSION, privacyVersion: PRIVACY_VERSION, marketingAgreed, ageOver14 }); setNotice('가입 확인 메일을 보냈습니다. 인증 후 로그인해주세요.'); setMode('login'); } else { sessionStorage.setItem('hokex-open-nickname-setup', 'true'); await signInWithPassword(email, password); } }
  ...
  return <div className="login-container"><div className="login-box"><div className="login-header"><h1>HOKEX</h1><p>전국 전시·컨벤션 정보 플랫폼</p></div><div className="login-content">
    ...
    <section className="social-consent" aria-label="가입 동의">
      <label className="checkbox-label"><input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} /><span><a href="/terms.html" target="_blank" rel="noreferrer">이용약관</a> 및 <a href="/privacy.html" target="_blank" rel="noreferrer">개인정보처리방침</a>에 동의합니다. <b>(필수)</b></span></label>
      <label className="checkbox-label"><input type="checkbox" checked={ageOver14} onChange={e => setAgeOver14(e.target.checked)} /><span>만 14세 이상입니다. <b>(필수)</b></span></label>
      <label className="checkbox-label"><input type="checkbox" checked={marketingAgreed} onChange={e => setMarketingAgreed(e.target.checked)} /><span>HOKEX 소식·이벤트·마케팅 정보 수신에 동의합니다. <b>(선택)</b></span></label>
    </section>
