-- Visitor Counter Tables - Clean Version (기존 객체 제거 후 재생성)
...
-- 2. 기존 함수 삭제 (존재하는 경우)
DROP FUNCTION IF EXISTS clean_expired_dedup_records();
DROP FUNCTION IF EXISTS reset_daily_visitor_counts();

-- 3. 기존 테이블 삭제 (존재하는 경우) - CASCADE로 연관 객체도 함께 삭제
...
CREATE INDEX idx_visitor_sites_domain ON visitor_sites(domain);
...
CREATE INDEX idx_visitor_dedup_site_hash ON visitor_dedup(site_id, visitor_hash);
CREATE INDEX idx_visitor_dedup_ttl ON visitor_dedup(ttl_expiry);

-- RLS Policies (Public access for visitor tracking)
ALTER TABLE visitor_sites ENABLE ROW LEVEL SECURITY;
ALTER TABLE visitor_logs ENABLE ROW LEVEL SECURITY;
ALTER TABLE visitor_dedup ENABLE ROW LEVEL SECURITY;

-- Allow public to read visitor counts
CREATE POLICY "Public can read visitor stats"
  ON visitor_sites FOR SELECT
  USING (true);

-- Allow Edge Function to insert/update (using service role)
...
-- Function to clean expired dedup records (optional, can be run via cron)
CREATE FUNCTION clean_expired_dedup_records()
RETURNS void AS $$
BEGIN
...
CREATE FUNCTION reset_daily_visitor_counts()
RETURNS void AS $$
BEGIN
...
BEGIN
  ...
  RAISE NOTICE '⚡ Functions created: clean_expired_dedup_records, reset_daily_visitor_counts';
  RAISE NOTICE '🚀 Next step: Edge Function already deployed at /functions/v1/track-visit';
END $$;
