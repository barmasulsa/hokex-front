-- Persist the 3:4 portrait thumbnail crop position selected by the author.
...
select id, title, content, board_category_id, author_nickname, created_at, updated_at,
       ...
       is_public, link_url, board_post_number, thumbnail_url, thumbnail_crop
...
drop function if exists public.create_community_post(text, text, text, text, text, boolean);
...
revoke all on function public.create_community_post(text, text, text, text, jsonb, text, boolean) from public;
grant execute on function public.create_community_post(text, text, text, text, jsonb, text, boolean) to authenticated;

drop function if exists public.update_community_post(uuid, text, text, text, text, text, boolean);
...
revoke all on function public.update_community_post(uuid, text, text, text, text, jsonb, text, boolean) from public;
