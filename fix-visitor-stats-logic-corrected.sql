...
-- 해결: ip_address + user_agent 조합으로 고유 방문자 카운트
-- ============================================
...
BEGIN
  ...
  SELECT 
    ...
    COALESCE(
      (
        SELECT COUNT(DISTINCT (vl.ip_address || '|' || COALESCE(vl.user_agent, '')))
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'stats', json_build_object(
      -- 오늘 고유 방문자 (ip + user_agent 조합)
      'today', (
        SELECT COUNT(DISTINCT (ip_address || '|' || COALESCE(user_agent, '')))
        FROM visitor_logs
...
-- 6. 검증 쿼리
-- 오늘 고유 방문자 수 확인 (ip + user_agent 조합)
SELECT 
  '오늘 고유 방문자' as label,
